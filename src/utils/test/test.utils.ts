...
export async function renderOptions<C>(
  args: Partial<RenderOptionsArgs> & { initialRoute: RouteLocationRaw }
): Promise<RenderOptions<C>>;
export function renderOptions<C>(
  args: Partial<RenderOptionsArgs> = {}
): RenderOptions<C> | Promise<RenderOptions<C>> {
  ...
  if (!initialRoute) return result as RenderOptions<C>;

  return new Promise((resolve) => {
    void router
      .replace(initialRoute)
      .then(() => resolve(result as RenderOptions<C>));
  });
}
